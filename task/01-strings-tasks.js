...
function getRectangleString(width, height) {
    if (width > 1, height > 1) {
        var s1 = '│' + ' '.repeat(width - 2) + '│\n',
            s = '┌' + '─'.repeat(width - 2) + '┐\n' + s1.repeat(height - 2) + '└' + '─'.repeat(width - 2) + '┘\n';
    }
    return s;
}
...
function getCardId(value) {
    var s = 0,
        s1 = 0,
        s2 = 0;
    if (value.charAt(value.length - 1) == '♣') s2 = 0;
    if (value.charAt(value.length - 1) == '♦') s2 = 13;
    if (value.charAt(value.length - 1) == '♥') s2 = 26;
    if (value.charAt(value.length - 1) == '♠') s2 = 39;
    if (parseInt(value.charAt(0), 10) > 1) s = parseInt(value.charAt(0), 10) + s2 - 1;
    else if (parseInt(value.charAt(0), 10) < 2) s = 9 + s2;
    else {
        if (value.charAt(0) == 'A') s = 0 + s2;
        if (value.charAt(0) == 'J') s = 10 + s2;
        if (value.charAt(0) == 'Q') s = 11 + s2;
        if (value.charAt(0) == 'K') s = 12 + s2;
    }
    return s;
}
